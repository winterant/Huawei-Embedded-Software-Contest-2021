...
def generate_in(filename, num_vertexes, C=1, num_stations=None, num_edges=None):
    ...
    with open(filename, 'w') as f:
        f.write(f'{num_vertexes} {num_edges} {C} {D}\n')

        # 前num_stations个设为基站，其余为卫星
        site = [(0 if i < num_stations else 1) for i in range(num_vertexes)]
        for i in range(num_vertexes):
            f.write(str(site[i]) + (' ' if i < num_vertexes - 1 else '\n'))

        # 一行基站/卫星的功耗系数
        for i in range(num_vertexes):
            f.write(str(random.randint(1, 10) if site[i] == 0 else random.randint(3000, 10000)) + (' ' if i < num_vertexes - 1 else '\n'))
